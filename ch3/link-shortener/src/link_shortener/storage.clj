...
(defprotocol Storage
  ...
  (create-link [this id url]
    "Saves the given URL under the id and returns the id if successful. Returns nill in case of failure 
    for example when the id is already in use.")
  (update-link [this id new-url]
    "Update an existing link to point to a new URL. Retuerns the id if the operation was successful, 
  ...
  (delete-link [this id]
    "Removes the link with the specified id from storage if it exists. Returns deleted link or nil otherwise")
  (reset [this]
